...
<body>
    <div class="container mt-5">
        <h1 class="text-center mb-4"><i class="fas fa-book-open icon"></i> Lección Completada</h1>
        ...
        <form method="post" action="/idiomify/ProgresoServlet?accion=agregarProgreso">
            <input type="hidden" name="FKidUsuario" value="<%=idUsuario%>">
            <input type="hidden" name="FKidIdioma" value="<%= leccion.getFKidIdioma() %>">
            <input type="hidden" name="FKidCurso" value="<%= leccion.getFKidCurso() %>">
            <input type="hidden" name="FKidLeccion" value="<%= leccion.getIdLeccion() %>">
            <input type="hidden" name="puntuacion" value="<%= puntuacion %>">
            <div class="d-flex justify-content-center">
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-arrow-circle-right icon"></i> Siguiente Lección
                </button>
            </div>
        </form>
    </div>
